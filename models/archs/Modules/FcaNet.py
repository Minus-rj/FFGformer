...
class MultiSpectralAttentionLayer(torch.nn.Module):
    def __init__(self, channel, dct_h, dct_w, reduction=16, freq_sel_method='top16', b=1, gamma=2):
    ...
    def forward(self, x):
        ...
        y = self.Conv1d(y.unsqueeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(
            -1)  # (B, C, 1)->(B, 1, C)--------->(B, 1, C, 1)
...
# facnet = MultiSpectralAttentionLayer(64,64,64)
# out = facnet(x)
# print(out.shape)

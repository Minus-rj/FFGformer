...
def align_single(img, h, w):  # x: [C,256,256], [0,1]
    x = img.clone()
    x = F.interpolate(x.unsqueeze(0), size=(h,w), mode='bilinear', align_corners=False).squeeze(0)
    # x = TF.normalize(x, mean=[0.5,0.5,0.5], std=[0.5,0.5,0.5])  # 与 Compose 里的 Normalize 完全一致
    return x
...
class VideoSameSizeDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print("self.data_info['folder']:", self.data_info['folder'])
        ...
        if self.opt['phase'] == 'train':
            ...
            # img_LQ = align_single(img_LQ, 256, 256)  #TRBID
            # img_GT = align_single(img_GT, 256, 256)  #TRBID
